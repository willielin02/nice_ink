...
void DrawBroadcastChrome(FCan& C, int32 Shot, float U, int32 FrameNo)
{
	...
	static const FTelop Telops[2] = {
		// 秒ではなく**割合**で書く：尺（IntroNoticeSeconds）は旋鈕なので、ここに秒を
		// 書き写した瞬間に腐る（08-29 実際に腐った——10.4s 時代の「0.2~3.3s」が残っていた）。
		{ NiceInkTvTelop::Txt_Ennichi, 0.02f, 0.05f, 0.29f, 0.32f }, // 段の 2~32%（夜祭→太鼓を跨ぐ）
		{ NiceInkTvTelop::Txt_Mikoshi, 0.35f, 0.38f, 0.50f, 0.53f }, // 段の 35~53%（神輿）
	};
