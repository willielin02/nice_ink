...
// --- 局內 HUD 尺度（基準 U = 4px @1080p）---
// **「全部都是 U 的整數倍」這句話不成立，別再照抄**（2026-09-19 查證）：`KeycapPad` 就是 1.5U＝6。
// 分界在**挑的**還是**推導出來的**：級距上的值（4／8／12／16／24／64）是挑的；6 是把 12px 的字
// 置中在 24 的方格裡剩下的 (24−12)/2，是幾何的結果。**推導出來的值可以落在格線外，挑的不行**
// ——挑一個格線外的數就是 §14 那一節（「你的排版準則到底是從哪裡來的」）在講的病。
...
namespace NiUi
{
	...
	// **ESC 首頁席位列專用的 12**（2026-09-19，user：「內容與框邊、內容之間的間距都改成 12」）。
	// 合法性：12 ＝ 3U（在 4px 網格上），也在 §14.1 引的 Carbon 級距表（2／4／8／12／16／24…）上。
	//（§14.5 那句「4／12／32 不在級距表上」是錯的——三個都在該節自己列的表裡，而 4 正是現行的 GapS。）
	// **為什麼需要一個自己的值，不能沿用 SeatPad 8**：8 是從大廳席位格抄來的，而那個格是 96×96
	// **裡面只有一顆臉**——8 在那裡是格子裡唯一存在的間距，沒有東西跟它比。09-18 把它拉成 560 寬、
	// 塞四樣東西的橫列之後，8 要跟一個大廳不存在的元件間距 24 並存 ⇒ **框內距︰元件間距 ＝ 1︰3，
	// 方向是反的**（同一列裡的臉／名字／錢／靴子屬於同一個人，框的邊界才是人與人的分界；照接近原則
	// 內距應該 ≥ 元件間距）。實拍讀感＝框咬著內容、內容彼此散開。**值抄過來了，前提沒抄。**
	// **兩個值，不同角色**（2026-09-19 二修，user：「內容之間的間距都改成 16；內容與框邊維持 12」）：
	// 一修時兩者同值 12，user 看過實拍後把元件之間拉開到 16、框邊留 12 ⇒ **組外 16 ＞ 組內邊界 12**。
	// 兩個都在級距表上（12＝3U／Carbon $spacing-04；16＝4U／$spacing-05）。
	constexpr float SeatRowPad   = 3.0f * U;   // 12 框邊 ↔ 內容
	constexpr float SeatRowGap   = 4.0f * U;   // 16 內容 ↔ 內容
	// （**不含「數字 ↔ 銭」**＝那是一個數值加它的單位、不是兩個成員，走 GapM 8；見 SNiHud 該處註解。）
	// 列高沿用 §13.6 五修的推導「列高不是設的，是臉＋兩個內距推出來的」⇒ 80 + 2×12 = 104。
	// **大廳的 SeatFrame 96 不動**：user 指的是 ESC 這一列；代價＝同一個「席位」在兩個畫面的框不再等大
	//（臉仍然都是 FaceSeat 80＝§13.6「同一個物件同一個尺寸」真正在講的那一個量）。
	constexpr float SeatRowFrame = FaceSeat + 2.0f * SeatRowPad;   // 104
