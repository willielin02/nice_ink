...
- `Source/NiceInk/`：`NiceInkCharacter`（輸入輪詢/貼臉鎖定/**直接畫制**（2026-07-20 起：
  ...
  **08-01 出墨層修＝拆 Shader 遺留移動閘＋首針即點**（3°/s 閘=慢畫整段無墨、
  ...
  改讀投影矩陣；貼邊判定生 aim→濾波 aim→四輪終判**可見筆尖網格投影**
  （出剪影時 P 凍結筆停畫面內、aim 續跑過帶=「還沒到邊就動、左右特別嚴重」
  真兇；鐵則=判可見物在哪必須投影可見物本身，上游代理必在某域分家）；
  可視畫布<可畫橢圓=結構事實、下一刀 FOV/眼距待裁）；Liner=**皮繩追趕巡航**（07-24 拉桿退役：滑鼠恆指哪、
