...
3. **「給我洞見/分析」= 只分析，不寫 SPEC。** 只有使用者明說「更新進專案」才動 SPEC。
   使用者沒說完的半句話不是設計決定。
   **但這條規則有對稱的另一半，2026-08-29 補上（血價：SPEC 停了十一天、落後五項定案）**：
   這條擋的是「我編的設計」，它**不擋、也不能擋「使用者真的定了案」**。所以——
   **使用者定案且實作落地後，帳本必須記一行「SPEC 待回寫：<哪一條>」**，累積到下次
   使用者授權對齊時一次寫進去。沒有這半條，預設行為就是定案全部堆在對話與帳本裡，
   SPEC 安靜地變成一份歷史文件——而它是**設計的唯一權威**，過期的權威比沒有權威更糟。
   同理：SPEC 的版本行要帶「與實作對賬」的日期，那個日期就是它的保鮮期。
   **對帳清單＝`Docs/SPEC_RECONCILE.md`**（2026-08-29 全文首跑，20 條可查證斷言各附查法）
   ——下次是重跑不是從頭來；跑完更新該檔末尾的紀錄與 SPEC 的日期戳。
   分類鐵則：**SPEC 舊、實作對**（使用者另有定案）⇒ 更新 SPEC；**實作漂了、沒有決定**
   ⇒ **不要改 SPEC，回報使用者**——拿改 SPEC 去蓋掉實作漂移＝把 bug 追認成設計。
...
- **常數要標明活在哪個座標系，而且它的前提要有閘門在看**（2026-08-31 血價，五輪）：
  `MarkerUvRadius` 活在 **UV 空間**，它等於 3.0mm 的前提就寫在自己的註解裡
  （「均勻密度圖集」）——**而沒有任何東西在檢查那個前提還成不成立**。
  推論鐵則：**兩個症狀互斥（怎麼調都是一邊好一邊壞）＝你在錯的層，解不在這個空間裡**，
  這要比「再修一次」先想到。
- **閘門必須能看見它要擋的那種失敗**（同批）：密度閘門量面積、失敗是各向異性
  ⇒ 它不是失效，是**從一開始就沒有解析度**（同族＝`stat unit` 的 Input 取樣週期
  大於測試窗）。寫閘門時要問：**什麼樣的壞會讓它照樣亮綠燈？**
- **改了上游資料，先問「誰把它算過一次存起來了」**（2026-08-31 最貴的一條）：
  `NiceInkFaceShare::BuildBlobFromDir` 的 `blob_nif2.bin` 只驗 magic、**不驗新舊**，
  註解寫著前提「library 資料夾按時間戳不可變」——而搬島必須改寫每一份
  `eye_mask_ink.png` ⇒ 08-14 的 blob 一路蓋過 08-31 修好的遮罩，**連續四輪修全部隱形**。
  已加「來源比快取新就重算」的閘門。**註解裡寫著前提的地方，就是遷移會撞爛的地方。**
- **「哪些資料錨在 UV0 上」必須包含執行期產物**（同批）：玩家上傳自拍時 FaceBakery
  現烤的眼罩住在 `Saved/PlayerFace/`，**而且每個沙箱各一份**（`Saved_P2/P3/P4`，
  受害者常常是別的視窗的角色）——re-import 永遠碰不到。
  **枚舉要走檔案系統，不要走記憶；連「掃描的根目錄」都不准手挑。**
- **robo 綠而 user 紅：先問「受測物是不是同一個」，再問「他那邊有沒有一層我沒有的快取」**
  （同批）：robo 用預設臉＝根本不走 blob 路徑，所以 robo 拍到的畫面是好的。
...
')` 後切片），完全繞開字串比對。
  **2026-09-03 同一輪踩了四次**——唯一有效的紀律是：**含轉義的腳本一律用 Write 寫成
  檔案再跑，不要用 heredoc 傳**。同族：改 .cpp 時字串比對常因縮排字元對不上而失敗，
  也走行號切片（先 `grep -n` ＋ `cat -A` 確認 tab 數）。
- **低解析度點陣字：有些字在這個尺寸上是畫不出來的，正解是換掉需要它的詞**
  （2026-08-29，8×9 テロップ）。マ 與 ス 結構完全相同（橫棒＋左下斜畫＋右下分枝），
  差別只在比例（第二畫短或長）——2px 對 4px 低於識別閾。五輪實測：畫第二畫→ス；
  不畫→**フ**（換成另一個字）；左臂拉滿到底→又是ス。**繼續畫那個字是在打不可能的仗**；
  改詞（ナツマツリ→エンニチ）十分鐘就結案。同案四條規則：把格子用滿（第一版只用 8 欄的
  中間 6 欄＝丟掉三分之一解析度）／筆劃間至少留一空像素／能直就不斜（8px 的斜線＝階梯
  ＝雜訊）／讓區別特徵最粗壯（先問「跟最像的字差在哪」）。**但「用滿」有例外——在
...
- **Canvas 畫圓不要用線段拼**（2026-09-03 血價，user：「為什麼會斷斷續續的」）：
  `DrawLine` 有兩個各自獨立的斷法——①線的端點是**平頭**（沒有 round cap）⇒ 相鄰段
  在轉角處露出楔形缺口 ②**不足一像素的線寬**在無抗鋸齒的光柵化下**整段被跳過**。
  正解＝`FCanvasUVTri` 的**實體環帶**：相鄰段共用同一組頂點 ⇒ 接縫在數學上不存在，
  sub-pixel 寬度被畫成部分覆蓋而不是消失，成本還一樣（一條 thick line 本來也是兩個
  三角形）。**要非線性的漸層就把曲線放進貼圖**（頂點顏色只能線性插值，而線性衰減的
  邊緣有可辨識的終點＝讀成「一條帶」不是「一團暈」）；貼圖驅動時三角形數量不變。
- **每幀成本會沿著離散步進洩漏進手感**（2026-09-03 血價，被 robo 抓到兩次）：
  作畫巡航的 `cruise tipSpd` 契約是 1.50~2.00 而**基線 1.99＝只有 0.5% 餘裕**；
  針以 v_max **逐幀**追趕游標，幀率一降每幀就走更遠、實走距離變長 ⇒ **HUD 多畫一點
  東西就會推移一個手感契約**。兩次都是「對小圈也做滿額工作」（28 段＋三次座標解算／
  多一層環帶）。**「兩輪穩定重現」就不可以再用 flake 解釋**——要嘛 `git stash push
  -- Source/` 重編譯驗基線、要嘛認帳。已知的 flake 清單是用來解釋「跳來跳去」的，
  不是用來解釋「穩定地變差」的。
- **`Canvas->K2_DrawPolygon` 會忽略你給的 alpha**（2026-09-03 血價）：它建的
  `FCanvasNGonItem` 沿用 `FCanvasItem` 的預設 **`BlendMode = SE_BLEND_Opaque`**
  ⇒ 半透明的填色畫成實心（user 截圖：「筆頭全部塗黑是什麼意思？」）。同一支 API
  在兇手轉盤用了一年沒出事，只因為那個盤底本來就要不透明——**能用不代表它做了你
  以為的事**。要半透明就自己建 item 設 `BlendMode`，別走 K2_ 便利函式。
...
- `Source/NiceInk/`：`NiceInkCharacter`（輸入輪詢/貼臉鎖定/**直接畫制**（2026-07-20 起：
  ...
  **作畫操作與 HUD 現制（2026-09-02，user 逐輪定案；帳本＝SHIP_PLAN 追記104~113、
  ...
  - **墨杯盤＝按住 RMB**（此前 RMB＝起身，與 WASD 重複＝浪費最貴的輸入；起身現在
    只有 WASD）。**恆為一排**（09-03）：稿筆 1 格（結晶紫）／割線 10 格／打霧 10 格
    ——**盤的形狀＝這支筆真正擁有的維度**（稿筆恆紫沒有顏色軸、濃度只有打霧吃得到）。
    **方向選擇、無游標**：位移鉗在盤內、取最近格 ⇒ 任何位移都落在某一格，
    不必對準；不動＝維持原杯＝天然的取消。放開＝沾杯（**只寫這支筆有的軸**——
    割線不得碰 `ShaderTierIdx`）。杯子用當前那一檔渲染、**盤開著時滾輪照樣調濃度**
    （整排一起變淡＝「濃度是另一個軸」由變化講完）。
    **原 10×3 已廢**：二維方向選擇會讓「往右換個顏色」順手改掉濃度，而濃度誤用
    **單向不可逆**——把不可逆的軸放在會被順手掃過的維度上＝製造無法挽回的誤觸。
  - **滾輪＝稀釋度**（**僅打霧筆**；作用域外連操作表那一行都不列）、**Q＝切筆**。
    檔位＝`77/153/255`＝**30%/60%/100%**（153 才是剛好 0.600；顯示一律走
    `TierLabel()` 單一來源）。**數字鍵 1-0 選色已於 09-03 退役**——顏色↔數字是
    任意映射（只能背）＝熟練度分級通道，而這是派對遊戲；對照組滾輪→濃度留著，
    因為那是自然映射。**差別是「要不要背」，不是「快不快」。**
  - **針尖指示器＝三支筆共用一個空心圓環**（09-03）：半徑＝那支筆真正的落墨半徑
    （打霧 1cm／稿筆・割線 1.5mm）⇒ 換筆時圈的大小就是筆寬；主線＝當前墨色，
    內外各一圈**指數衰減的暈**（內暗外亮，曲線住在 runtime 生成的 64×1 貼圖裡
    ＝三角形數量不變）。**必須是實體三角形環帶，不能用線段拼圓**（見陷阱年鑑）。
    暈屬於「自己製造背景」⇒ **不必知道底色**，為了讀底色而建的亮度圖整套已拆除。
  ...
  **開場動畫（2026-08-27~28 BUILT-自驗、待 viewport；帳本=OPENING_CEREMONY_PLAN 08-27 節＋SHIP_PLAN 追記85）**＝user 逐字定案「坐在榻榻米上喝酒看電視→電視上極道刺青→關電視→有人提議→開局」：`ENiCeremonyStep` 在 Gather 前插五拍 IntroSit/Notice/TvOff/Propose/Rise（**儀式步驟全體 +5＝robo 腳本寫死的步驟號已同步修**）；只在本房第一場播（`bOpeningIntroPlayed`）、**PIE 一律跳過**（robo 契約零擾動；自驗走 `bOpeningIntroForceInPIE`＋`robo_intro_shots.py`）；入座＝相位切換當幀 teleport（與導演鏡頭硬切同幀＝看不見搬運）；坐姿＝SitBones 第一個消費者（`ComposeSitBaseCS`）、席弧依在場名次置中（24°@r260＝109cm＞體寬；17° 會互穿）；**坐→站不插值＝Propose→Rise 剪接間 snap**（中間幀只有被拍到才存在）；分拍機位表`ViewIntro`（blend=0 真剪接、機位內單調緩推；反拍在並排坐時只拍到背影＝砍掉）；電視 `ANiceInkTvSet`＝**SM_TvRadiola**（"Radiola from Matrix" by Sirenko，Sketchfab CC-BY-4.0 可商用需署名；SourceAssets/Television_Sirenko/ATTRIBUTION.txt、信用已列選單授權頁＋THIRD_PARTY_NOTICES；管線=tv_radiola_prep.py→ue_import_television.py）＋**節目＝日本傳統祭典特輯（08-28 user 定案：低解析度舊電視／祭典／誇張帥氣的極道／讓力士羨慕刺青）**：影像＝128×96 CPU 影格（`NiceInkTvBroadcast`）→ `TF_Nearest` 貼圖 → 單一 DrawTile **整數 4×** 進 512×384 CanvasRT（掃描線／陰罩／暗角／關機白線留在 512 那層＝兩個解析度）；五分鏡＝`ResolveShot(step,alpha)` 純函式（夜祭／太鼓／神輿／主役登場／振り返り）——**IntroSit（全景）只負責「電視是開著的」，IntroNotice（特寫 **15.6s**）把五分鏡從頭全部演完**（08-28 user 指定「讓玩家可以在電視前完整看完」；一修那版有三個分鏡只在全景播＝電視只有 20px＝做了看不到的東西）。**戲劇進度(U) 與物理速度(Seconds) 是兩條路**：U 管緩推／滑落／字幕／キラッ（鏡頭拉長就整體變慢＝正確），Seconds＝實秒管太鼓的拍／提燈的搖（不隨鏡長變——一修用 U 當相位，鏡頭拉長四倍太鼓就變 1/4 速）；分鏡佔比用比率表 ⇒ 長度只有 `IntroNoticeSeconds` 一個旋鈕。開場全長 **25.3s**（08-29 user 要求影片 +50%＝10.4→15.6s，**只改 `IntroNoticeSeconds` 一個數字**——分鏡比率表自動等比、物理速度由 Seconds 側保住；本房一次；`BeginOpeningIntro()` 的 Total 自動吸收相位計時器）。開場期間 **HUD 全部讓開**（`NiCeremonyStepIsIntro` 早退，只留 ESC 選單）；**08-29 改判為新聞畫面**（user：「這要營造的是新聞畫面比較合理」——劇情片的帥是虛構的、新聞裡的男人是真的存在的，錯誤推論才站得住）＝下三分之一テロップ（**一段 12px＋紅色見出し「生」、寬度跟著內容、ワイプ出入**）／3×5 點陣時刻＋生中継バッジ（**數字是這個解析度上唯一真的能讀的字**）／手持晃動（**畫面晃、chrome 不晃**⇒ 順序＝畫→晃→chrome→汚れ）／膠片假影換成錄影帶假影（**剪接白閃在新聞裡原理上不存在**——追記86 把電影放在電視上了）／重畫率 12→24（`RedrawHz` 單一來源；12Hz 本身就是膠片的選擇）／記者剪影／キラッ→レンズフレア。連鎖＝テロップ 佔 y74~93 會把担ぎ手的頭埋掉讀成柵欄 ⇒ 三拍構圖整個上抬（**報道のカメラマンは下三分之一があることを知って構図を切る**）；**三修（user viewport：「標題太大、文字看不出在寫什麼、完全不像日本字」）**＝帶高 19→12px（二段是速報作法、祭典特輯過剩）＋帶寬跟著內容走（全幅帶讀起來是「貼上去的板子」）＋假字塊改成**真的點陣片假名**（`Tools/AssetPrep/telop_glyphs.py` 烘 → `NiceInkTvTelopData.h`；**手工點字是這個尺寸的正解**——9px 上 rasterize 向量字型必糊，而且不碰字型授權；片假名是直線構成、8×9 唯一站得住的日文字，漢字只收「生」）；**マ 的血價＝有些字在這個尺寸畫不出來，正解是換掉需要它的詞**（マ 與 ス 在 8×9 結構相同只差比例；帶第二劃→ス、整劃拿掉→**フ**、左臂拉滿→又是ス，五輪皆敗。改詞 ナツマツリ→エンニチ 十分鐘結案；マ 已標 `UNRELIABLE` 由 baker assert 擋住）；儀器＝telop_glyphs.py 同時吐標頭與自查 PNG、**不用開引擎、一輪 10 秒**；背中の主図仍是 **DreamTrace motif 資料原樣＝電視上的圖＝夢裡要描的圖**（改用輪廓，實心會變成白色星形 logo）；色域＝sRGB byte 40~255（unlit×8 下純黑讀成螢幕破洞）；**現行字幕＝整段兩條、都跨剪接點**（テロップ 屬於報導不屬於鏡頭；掛在段全體進度上，不掛 (Shot,U)——掛鏡頭會因為分鏡只有 2~3 秒而結構性地每兩秒閃一次）：生 エンニチ 0.3→5.0s／生 ミコシ 5.5→8.3s／**其後 7.3s 完全乾淨（主役兩拍零 chrome，彫物全程無字幕——字幕指著它就變廣告，也替力士把結論講完了）**。儀器＝`robo_tv_filmsheet.py` 離線 contact sheet＋45 張逐格（不用開 PIE、一輪 60 秒）。畫在模型自己的 `TvGlass` 材質槽上（內凹圓角玻璃 52.5×44.7、UV 歸一化到島；浮貼平板退役）；AO 接回櫃體材質、熄滅玻璃灰綠、RT 邊緣映像管暗角＝框/玻璃邊界對比（user 三輪打回定案）；開場後電視留場當家具。**v1 記帳未做**：房主手上無機器道具網格（舉拳＋marker_loop 補位）、坐姿飲酒、電視音效（~~開場期間 HUD 仍顯示 BOTTLE SPIN 倒數~~＝08-28 已修：**記帳過的缺口會隨著別處的改動變質**——特寫從 2.6s 拉到 10.4s（現 15.6s）之後，釘在畫面正中央的準星就整整十秒壓在電視螢幕上）)、
...
- 墨水圖集 UV0＝**均勻紋素密度**（sumo 實測 0.617 px/mm；RT 解析度 4096＝筆寬 4.7px
  ＋線層 Valve alpha 銳化）；筆寬 `MarkerUvRadius 0.000452`＝3.0mm 全身一致（08-02 user 定值；**RT 4096 下 1 texel=0.81mm、線寬=3.70 texel**）；
  **⚠「均勻」有例外，2026-08-31 才補上（血價：user 五輪回報乳頭爆墨／斷墨）**：
  乳頭×2／乳暈×2／肚臍是 retopo 為**保住解剖形狀**留的獨立連通元件（各自封閉、零邊界邊）
  ⇒ 各自成 UV chart ⇒ **逃出 `uv0_uniform` 的作用域**。而它的密度閘門量 `sqrt(Auv/A3)`＝
  **面積**比，對「單方向被壓扁」天生看不見（島的面積密度是身體的 1.01~1.27 倍＝一直報成功）。
  真相要量 **Jacobian 奇異值**：修前乳暈 28% 的面積上一條 3mm 的線**不到一個紋素寬**
  ⇒ 常數 UV 半徑＝12~14mm 的章（一坨）／實體 3mm＝0.8 紋素（細又斷），**兩個症狀互斥、
  筆寬旋鈕裡沒有解**。現制＝`sumo_anat_uv_fix.py` 只重展搬這五顆島（66°，其餘 chart 逐位不動、
  島∩身體重疊 0 紋素），p10 0.90→2.67~3.30、<2紋素 23.5%→0%、charts 43→39。
  **搬島必跑的兩支**：`ink_uv_anchor_scan.py`（全樹枚舉哪些貼圖錨在 UV0、不准手列清單）
  ＋`sumo_anat_uv_retarget.py`（把內容搬過去；含 `Saved*/PlayerFace` 執行期烘出來的臉）。
  驗收量＝`ink_texel_floor.py`（一條 3mm 的線有幾個紋素寬，身體基準 p10 2.82）。
  線的跨縫
