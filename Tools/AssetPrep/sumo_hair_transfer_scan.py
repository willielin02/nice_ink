# 掃描髮貼圖轉印——幾何端（Blender）
# 每個 UV0 髮區紋素：3D 位置 → BVH 最近掃描表面點 → 重心插值掃描 UV
# 輸出 npy 給 venv 端 cv2.remap 取樣（scan_Image_0.png，三張 basecolor 已驗證相同）
import bpy
import numpy as np
...
# ---- 髮區面集合（同 sumo_hair_dump_tris：頂點級補洞＋拉鏈）----
...
for tri in me.loop_triangles:
    ...
    for li in tri.loops:
        ...
        pr += [p.x, p.y, p.z]
    hair_tris.append((uvr, pr))
print(f"HAIR tris={len(hair_tris)}")
...
print(f"SCAN appended: verts={len(sme.vertices)} tris(polys)={len(sme.polygons)}")
...
# ---- 平移 ICP：master 頭區點 → scan 表面（scan+t ≈ master ⇔ 查詢 p−t）----
...
# 初始猜測：z 最低點對齊（master 腳底 0 vs scan 腳底）
...
for vi in marked:
    ...
    co, nrm, idx, dist = bvh.find_nearest(p - t)
    if co is not None:
        hair_res.append(dist)
hair_res.sort()
print(f"HAIR residual mean={sum(hair_res)/len(hair_res)*1000:.2f}mm "
      f"p95={hair_res[int(len(hair_res)*0.95)]*1000:.2f}mm max={hair_res[-1]*1000:.2f}mm")

# ---- 髮區紋素 → 掃描 UV ----
uv_map = np.zeros((SIZE, SIZE, 2), np.float32)
cover = np.zeros((SIZE, SIZE), bool)
dists = []
for uvr, pr in hair_tris:
...
np.save(S + r"\hair_scanuv.npy", uv_map)
np.save(S + r"\hair_cover.npy", cover)
print("TRANSFER_GEO_DONE")
