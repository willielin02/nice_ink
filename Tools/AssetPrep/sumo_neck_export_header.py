...
def ring_color(v):
    ...
    for e in v["uv"]:
        n = e["n"]
        hr = sample(hair_t, e["hairuv"][0], e["hairuv"][1])   # sRGB
        m = sample(hairm_t, e["hairuv"][0], e["hairuv"][1])[0]
        for k in range(3):
            acc_hr[k] += srgb_to_lin(hr[k]) * n
        hm += m * n
        cs += n
    hr = [c / cs for c in acc_hr]
    hm /= cs
...
for v in me.vertices:
    for g in v.groups:
        if g.group == gi_head and g.weight > 0.5:
            ...
            break
assert len(shell) == 1310, f"head shell {len(shell)} != 1310"
...
L.append("// 邊界環＝user 手標 cut_seam_head（72 頂點）切開後的頭側/身側環（UE component space, cm）。")
...
L.append("{")
L.append("constexpr int32 RingCount = 72;")
L.append("")
