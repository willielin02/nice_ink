...
start "" %UE% %PROJ% /Game/Maps/L_Dojo?listen -game -windowed -resx=960 -resy=540 -WinX=20   -WinY=40  %NETARG%
rem give the listen server time to bind before the clients connect
timeout /t 12 /nobreak >nul
start "" %UE% %PROJ% 127.0.0.1 -game -windowed -resx=960 -resy=540 -WinX=1000 -WinY=40  %NETARG%
rem stagger the remaining clients so they don't all hammer load at once
timeout /t 4 /nobreak >nul
start "" %UE% %PROJ% 127.0.0.1 -game -windowed -resx=960 -resy=540 -WinX=20   -WinY=620 %NETARG%
timeout /t 4 /nobreak >nul
start "" %UE% %PROJ% 127.0.0.1 -game -windowed -resx=960 -resy=540 -WinX=1000 -WinY=620 %NETARG%
