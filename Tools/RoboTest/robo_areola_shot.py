...
OUT = "C:/games/Unreal Engine/nice_ink/Saved/robo_areolashot_result.txt"
CSV = "C:/games/Unreal Engine/nice_ink/Saved/robo_areolashot.csv"
DUMP = "C:/games/Unreal Engine/nice_ink/Saved/robo_as"
LINES = []
...
class Probe:
    ...
    def step(self):
        ...
        if s == "boot":
            ...
            self.sweep_i += 1
            self.advance("sweep_prep" if self.sweep_i < len(SWEEPS) else "shot_focus")
        elif s == "shot_focus":
            # HighResShot 只有聚焦視窗會處理；截圖下一幀才落地
            import ctypes
            try:
                ...
                pass
            # 照抄 repo 既有做法（robo_directdraw_test / robo_collapse_jiggle）
            unreal.SystemLibrary.execute_console_command(
                self.server(), "HighResShot 1280x720 filename=areola_after")
            log("SHOT issued")
            self.advance("shot_wait")
        elif s == "shot_wait":
            if self.elapsed() < 2.0:
                return
            self.advance("done")
