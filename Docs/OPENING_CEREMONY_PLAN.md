...
### 2026-08-28 電視節目改制：日本傳統祭典特輯（全文＝SHIP_PLAN 追記86）

user 定案（逐字）：「電視機中的畫面是低解析度、像是真的舊電視的畫面，播放著日本傳統
祭典影片，祭典中有非常帥（甚至有點誇張、典型的帥哥刻板印象）的極道人士，讓力士們羨慕
他們的刺青，覺得刺上一樣的刺青就可以變一樣帥」。

| 節目分鏡 | 儀式拍 | 秒 | 內容 |
|---|---|---|---|
| 夜祭遠景 | IntroSit α<0.30 | 1.2 | 櫓・提燈三列・人群・火の粉 |
| 太鼓 | IntroSit 0.30~0.58 | 1.1 | 裸上身の男二人＋大太鼓（肩に彫物＝伏線） |
| 神輿渡御 | IntroSit α>0.58 | 1.7 | 横搖 pan；担ぎ手の腕に彫物 |
| **主役登場** | IntroNotice α<0.50 | 1.3 | 半纏が肩から滑り落ち、満背和彫が逆光に現れる |
| **振り返り** | IntroNotice α>0.50 | 1.3 | 横顔・墨鏡反光・叼菸・キラッ（誇張の記号） |

- **導演權在儀式時間軸上，不在牆鐘上**：節目的剪接點壓在玩家的剪接點上
  （IntroSit→IntroNotice 那一刀＝鏡頭剪到電視特寫的同一幀，節目也剪到主役
  ＝「他們就是這一刻注意到的」）。`ResolveShot(step, alpha)` 是純函式，零複製欄位。
- **兩個解析度**：影像＝128×96 CPU 緩衝→`TF_Nearest` 貼圖→單一 DrawTile 放大 4×
  （硬邊方塊像素）；掃描線／陰罩／暗角／關機白線留在 512 那層。關機時被壓成線的是
  **最後那一格影像本身**，不是黑帶。
- **主題閉環維持**：背中の主図＝DreamTrace motif 資料原樣（電視上的圖＝夢裡要描的圖）。
- 儀器：`Tools/RoboTest/robo_tv_filmsheet.py`（離線 contact sheet，**不用開 PIE**、
  一輪 60 秒＝低解析度美術的迭代閘門）；`robo_intro_shots.py` 特寫拍加成三點。
